...
package net.mcreator.nkquest.world.structure;
...
public class StructureFlashingMushroom2 extends ElementsNkquest.ModElement {
	public StructureFlashingMushroom2(ElementsNkquest instance) {
		super(instance, 347);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((random.nextInt(1000000) + 1) <= 900000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("nkquest:magmatishroommoor")))
					biomeCriteria = true;
				if (!biomeCriteria)
					continue;
				if (world.isRemote)
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("nkquest", "flashingmushroom2"));
				if (template == null)
					return;
