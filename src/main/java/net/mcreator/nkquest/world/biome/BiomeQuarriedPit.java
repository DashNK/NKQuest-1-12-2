...
public class BiomeQuarriedPit extends ElementsNkquest.ModElement {
	@GameRegistry.ObjectHolder("nkquest:quarriedpit")
	public static final BiomeGenCustom biome = null;
	public BiomeQuarriedPit(ElementsNkquest instance) {
		super(instance, 645);
	}
	...
	public void init(FMLInitializationEvent event) {
		BiomeDictionary.addTypes(biome, BiomeDictionary.Type.PLAINS, BiomeDictionary.Type.RARE);
		BiomeManager.addSpawnBiome(biome);
		BiomeManager.addBiome(BiomeManager.BiomeType.WARM, new BiomeManager.BiomeEntry(biome, 1));
	}
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Quarried Pitlands").setRainfall(0.5F).setBaseHeight(0.1F).setHeightVariation(0F).setTemperature(0.5F));
			setRegistryName("quarriedpit");
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.STONE.getStateFromMeta(0).getBlock() || ground == Blocks.STONE.getStateFromMeta(0).getBlock())
							&& (ground2 == Blocks.STONE.getStateFromMeta(0).getBlock() || ground2 == Blocks.STONE.getStateFromMeta(0).getBlock())))
						return false;
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.AIR.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == Blocks.STONE.getStateFromMeta(0).getBlock()
