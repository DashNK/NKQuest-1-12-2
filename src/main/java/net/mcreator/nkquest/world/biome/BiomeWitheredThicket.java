...
public class BiomeWitheredThicket extends ElementsNkquest.ModElement {
	@GameRegistry.ObjectHolder("nkquest:witheredthicket")
	public static final BiomeGenCustom biome = null;
	public BiomeWitheredThicket(ElementsNkquest instance) {
		super(instance, 305);
	}
	...
	public void init(FMLInitializationEvent event) {
		BiomeDictionary.addTypes(biome, BiomeDictionary.Type.FOREST, BiomeDictionary.Type.DEAD, BiomeDictionary.Type.SPOOKY,
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Withered Thicket").setRainfall(0.5F).setBaseHeight(0.1F).setWaterColor(-16701914)
					.setHeightVariation(0.2F).setTemperature(0.5F));
			setRegistryName("witheredthicket");
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.GRASS.getDefaultState().getBlock() || ground == Blocks.DIRT.getStateFromMeta(1).getBlock())
							&& (ground2 == Blocks.GRASS.getDefaultState().getBlock() || ground2 == Blocks.DIRT.getStateFromMeta(1).getBlock())))
						return false;
					...
					if (position.getY() < world.getHeight() - height - 1) {
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.LEAVES2.getStateFromMeta(1).getBlock()
									|| state.getBlock() == Blocks.LEAVES2.getStateFromMeta(1).getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), Blocks.LOG2.getStateFromMeta(1));
								if (genh > 0) {
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(-1, genh, 0)))
										this.setBlockAndNotifyAdequately(world, position.add(-1, genh, 0), Blocks.LEAVES2.getStateFromMeta(1));
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(1, genh, 0)))
										this.setBlockAndNotifyAdequately(world, position.add(1, genh, 0), Blocks.LEAVES2.getStateFromMeta(1));
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(0, genh, -1)))
										this.setBlockAndNotifyAdequately(world, position.add(0, genh, -1), Blocks.LEAVES2.getStateFromMeta(1));
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(0, genh, 1)))
										this.setBlockAndNotifyAdequately(world, position.add(0, genh, 1), Blocks.LEAVES2.getStateFromMeta(1));
								}
							}
		...
		private void addVines(World world, BlockPos pos) {
			this.setBlockAndNotifyAdequately(world, pos, Blocks.LEAVES2.getStateFromMeta(1));
			int i = 5;
